    ...
    for ii, imdir in enumerate(imdirs):
        ...
        for i in range(int(num_rows)):
            ...
            for j in range(int(num_cols)):
                ...
                # Nl, Wl = geotransform_label2prod(Wl, Nl)
                Nij = Nl - i * 10 * sample_size  # N for extracted label window
                Wij = Wl + j * 10 * sample_size  # W for extracted label window
                Wij, Nij = geotransform_label2prod(Wij, Nij)
                ip = (Np - Nij) / (10 * sample_size)  # product row
                jp = (Wij - Wp) / (10 * sample_size)  # product column

                # exception: image id falls outside sentinel product
                if (ip < 0) or (jp < 0):
                ...
                # exception: no labels for this location
                if labels[i * label_mult * sample_size: (i + 1) * label_mult * sample_size, j * label_mult * sample_size: (j + 1) * label_mult * sample_size].sum() == 0:
                    saved_files_info.append(
                ...
                # none of the above exceptions apply, save image data
...
def main():
    ...
    # read all extracted ground truths files
    ...
    # get information on saved sentinel products
    ...
    for year in set(years):
        ...
        num_rows, num_cols = [d / (10 / res * sample_size) for d in labels.shape]
